...
void empty_string_test() {
    ...
    const uint32_t num_bytes = SHA256_BITS / 32;
    uint32_t result[num_bytes];
    char result_str[SHA256_STR_LEN];
    result_str[SHA256_STR_LEN - 1] = '\0'; // null terminate the result string
    ...
    sha_string(result, num_bytes, result_str);
